...
export default class Sqlite3Logger {
  ...
  public constructor(path:string, verbose:boolean) {
    ...
    this._dbPromise = new Promise<sqlite3.Database>((resolve, reject) => {
      const db = new sl.Database(Sqlite3Logger.path, (err) => {
        ...
        db.serialize(() => {
          db.exec(SQL.logTable, () => {console.log('log table created')});
          db.exec(SQL.logLevelTable,() => {console.log('log leve ltable created')});
  ...
  public async Log(level: LogLevelType, tag: string, message: string): Promise<void> {
