...
const p1 = (async function () {
  ...
  try {
    ...
    await bl.error(
      ['demo', 'test', 'error', ' ALL_TAGS_ARE_NORMALIZED_TO_lowercase'],
      `This is an error message. it gets ${Date.now()} highlighted as an error`,
    );

    await bl.warn(
      ['demo', 'test', 'warn', 'spaces are ok'],
      `This is a warning message. it gets highlighted as a warning ${Date.now()}`,
    );
  } catch (er) {
    const _null = '[null]';
  ...
  try {
    //bl.hush();
    await bl.info(
      ['demo', 'hush test', 'hush-test'],
      `This is the hush demo. When you hush, it silences all output to the console. If you call hushNext(), it will only silence the next log message.`,
    );
    await bl.debug(
      ['demo', 'hush test', 'hush-test'],
      'The hush method will continue across all log levels until you call unhush().',
    );
    await bl.error(
      ['demo', 'hush test', 'hush-test'],
      'This includes error messages. Again, all of these still go to the persistent log.',
    );
    //bl.unhush();
    await bl.info(
      ['demo', 'hush test', 'hush-test', 'unhush'],
      `Since Unhush has been called, this will go to the console.`,
    );
  } catch (er) {
    const _null = '[null]';
    console.error(er?.toString() || _null);
...
const p2 = (async function () {
  ...
  try {
    ...
    ee.on('tick', () => {
      try {
        bl.warn(['tick', 'warn', 'no-close'], `'no-close' event emmiter tick log `).catch(er =>
          console.error(er?.toString()),
        );
      } catch (er) {
        console.error(er?.toString());
    ...
    for (let i = 0; i < loops; i++) {
      ee.emit('tick');
      await bl.debug(
        ['demo', 'loop-test', 'no-close'],
        `no-close Loop test ${i} of ${loops} (${i / loops}) ${hushToggle}`,
      );
    }
  } catch (er) {
    const _null = '[null]';
